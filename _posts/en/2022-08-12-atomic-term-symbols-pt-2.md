...
Instead of $S$, we denote the **spin multiplicity**, which is equal to $2S+1$. As $M_S = -S, \,-S+1,\, \cdots ,\,+S-1,\,+S$, $2S+1$ is in fact the total number of $M_S$. Keep in mind that $M_L = -L, \,-L+1,\, \cdots ,\,+L-1,\,+L$, and that a total of $2L+1$ different $M_L$ are possible for a particular value of $L$.
...
The third and fourth microstate are a bit different. The third microstate, with the $1s$ electron spin up and the $2s$ electron spin down, results in two plausible wave functions: $1s(1)\alpha(1)\,2s(2)\beta(2)$ and $2s(1)\beta(1)\,1s(2)\alpha(2)$. Therefore, $1s(1)\alpha(1)\,2s(2)\beta(2) - 2s(1)\beta(1)\,1s(2)\alpha(2)$ is a suitable antisymmetric wavefunction. The fourth microstate, with the $1s$ electron spin down and the $2s$ electron spin up, results in $1s(1)\beta(1)\,2s(2)\alpha(2)$ and $2s(1)\alpha(1)\,1s(2)\beta(2)$. Likewise, $1s(1)\beta(1)\,2s(2)\alpha(2)-2s(1)\alpha(1)\,1s(2)\beta(2)$ is a suitable antisymmetric wave function.

![Microstate 3 and 4](/assets/img/atomic-term-symbols/microstates_helium_5.png)

However, linear combinations of both wave functions, $1s(1)\alpha(1)\,2s(2)\beta(2) - 2s(1)\beta(1)\,1s(2)\alpha(2) + 1s(1)\beta(1)\,2s(2)\alpha(2) - 2s(1)\alpha(1)\,1s(2)\beta(2)$ and $1s(1)\alpha(1)\,2s(2)\beta(2) + 2s(1)\beta(1)\,1s(2)\alpha(2) - 1s(1)\beta(1)\,2s(2)\alpha(2) - 2s(1)\alpha(1)\,1s(2)\beta(2)$, will be used for now. This is because the new wave functions are eigenfunctions of $\hat{H}, \hat{L}^2, \hat{L}_z, \hat{S}^2, \hat{S}_z$ simultaneously, while the two old wave functions were not.
...
For all four microstates, $M_L = 0$ as $m_l$ for both $1s$ and $2s$ orbitals are 0. We can organize these microstaes into a table. They are given as $\left( 0^+, 0^- \right)$, which means $m_{l1} = 0, m_{s1} = +1/2, m_{l2} = 0,$ and $m_{s2} = -1/2$.

![Microstate Table](/assets/img/atomic-term-symbols/table.png)

We will now obtain the term symbols by progressively erasing the (micro)states of each term symbol.

From these values of $M_L$ and $M_S$, note that the maximum value of $M_S$ is +1 and the minimum value is -1. Assume for a moment that $S$ is larger than 2. Then, the maximum value of $M_S$ should be larger than 2, and the minimum value of $M_S$ should be smaller than -2. As that is not the case, $S$ is not larger than 2, and as a consequence, the largest $S$ possible must be 1.

As the maximum value of $M_L$ is 0, it must be that $L = 0$. Thus, the largest possible combination of $L$ and $S$ is $L = 0$ and $S = 1$. This is $^3S$ in term symbol notation, which consists of three states: $M_L = 0$ and $M_S = -1, \,0, \,+1$. After confirming the term symbol $^3S$, we erase three states from the table: one from each possible $M_L$ and $M_S$ to find other term symbols with smaller $L$ and $S$.

![Table, crossed out](/assets/img/atomic-term-symbols/table_cross.png)

For the entries in $M_L = 0$ and $M_S = 0$, the microstate $\left(0^+, 0^-\right)$ is erased instead of $\left(0^-, 0^+\right)$. This does not mean that $^3S$ contains the microstate $\left(0^+, 0^-\right)$; it merely means that one of two states with $M_L = 0$ and $M_L = 0$ is in $^3S$, while the term symbol for the other state is yet to be determined. The most technically accurate way to erase these states would be erase one-half of both states, but that can result in confusion. Instead, a single state (either one) is completely erased for visual clarity, and the choice of microstate to be erased is unimportant. 
...
If $J_{max} = j_1 + j_2$, then what is $J_{min}$? The smallest maximum value of $M_J$ is given by the difference of the largest values of $m_{j1}$ and $m_{j2}$. This is equal to the difference between $j_1$ and $j_2$.
...
Let us apply this for the $L$ and $S$ of $1s^12s^1$. For the electron in the $1s$ orbital, $l_1 = 0$ ($s$ orbital). For the electron in the $2s$ orbital, $l_2 = 0$. Therefore, $L_{min} = \left\| 0 - 0 \right\| = 0$ and $L_{max} = 0 + 0 = 0$. This means that $L = 0$.
...
As a simple exercise, let us tackle $1s^12s^13s^1$. For this configuration, we split it into $1s^12s^1$ and $3s^1$. We know the $l_1$ & $s_1$ of $1s^12s^1$ - it is the same as our previous answer: $l_1 = 0$ & $s_1 = 0, 1$. For $3s^1$, $l_2 = 0$ ($s$ orbital) & $s_2 = 1/2$. There are two additions possible: (1) between $l_1 = 0$ & $s_1 = 0$ and $l_2 = 0$ & $s_2 = 1/2$, or (2) between $l_1 = 0$ & $s_1 = 1$ and $l_2 = 0$ & $s_2 = 1/2$.

For (1), $l_1 = 0$ adds with $l_2 = 0$ to produce $L_{min} = \left\|0 - 0\right\| = 0$ and $L_{max} = 0 + 0 = 0$, resulting in $L = 0$. This is unsurprising as all electrons are in an $s$ orbital. $s_1 = 0$ and $s_2 = 1/2$ results in $S_{min} = \left\|0 - \frac{1}{2}\right\| = \frac{1}{2}$ and $S_{max} = 0 + \frac{1}{2} = \frac{1}{2}$. Therefore, $L = 0$ and $S = 1/2$, and the term symbol is $^2S$.
...
Note that for elements with atomic number less than 30, the effect of $\hat{H}^{(1)}$ is much smaller than $\hat{H}^{(0)}$. As a result, $\hat{L}^2$, $\hat{L}_z$, $\hat{S}^2$, and $\hat{S}_z$ only *nearly* commutes with $\hat{H}$, and an energy difference of 

$$E^{(1)} \approx \frac{1}{2}A[J(J+1) - L(L+1) - S(S+1)]$$

for multi-electron atoms. This breaks the degeneracy previously seen with $L$ and $S$. If so, what is $J$? For starters, $\bold{J}$ is the total angular momentum, which is equal to $\bold{L} + \bold{S}$. In operator form, 
...
Therefore, $L$, $S$, $J$, and $M_J$ are used to specify an energy state instead of $L$, $M_L$, $S$, and $M_S$. To specify an energy level (term symbol), $L$, $S$, and $J$ (but not $M_J$, due to the lack of $M_J$ in $E^{(1)}$ above) are required.
...
As $L$, $S$, and $J$ are all required to specify an energy level, the term symbol notation changes from $^{2S+1}L$ to

> $$ ^{2S+1}L_J $$

The degeneracy of this term symbol is now $2J+1$, instead of $\left(2L+1\right)\left(2S+1\right)$.

For $L = 1$, $S = 1$, and $J = 0, 1, 2$, the new term symbols are $^3P_0, ^3P_1$, and $ ^3P_2$ instead of just $^3P$. In other words, the 9 degenerate $^3P$ energy states are actually 1 state belonging to $^3P_0$, 3 degenerate states belonging to $^3P_1$, and 5 degenerate states belonging to $^3P_2$. As demonstrated, the number of states do not change. For a given $L$ and $S$,
...
Approach 2: for equivalent electrons[^3]
...
[^1]: In some textbooks (such as *Levine*), a *term* refers to an energy level (determined by electron configuration, $L$ and $S$), while a *term symbol* refers to the symbol that is used to represent the term.
...
[^3]: Also possible for non-equivalent electrons.