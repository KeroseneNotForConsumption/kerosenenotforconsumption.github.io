...
The molecular orbitals (MOs) of homonuclear diatomic molecules can be constructed with the Linear Combination of Atomic Orbital method (LCAO). Let's take $\text{F}_2$ as an example.
...
$2p_{x}$ and $2p_{y}$ orbitals can form either bonding MOs $(\pi_{u2p_{x}}$ and $\pi_{u2p_{y}})$ or antibonding MOs $(\pi_{g2p_{x}} ^{\ast}$ and $\pi_{g2p_{y}} ^{\ast})$.
...
Lastly, the $2p_z$ can form either a bonding MO $(\sigma _{g2p_z})$ or an antibonding MO $(\sigma _{u2p_z}^{\ast})$. 
...
After filling in the MOs with electrons according to the Aufbau principle, the correlation diagram is complete.

![O2 correlation](/assets/img/s-p-mixing/correlation-o2.png)

The correlation diagram of $\text{O}_2$ is similar. However, for other second-period elements $(\text{Li}$ to $\text{N})$, the order among $\pi _{u2p_x} / \pi _{u2p_y}$ and $\sigma _{g2p_z}$ is **reversed**: the energy of $\pi _{u2p_x} / \pi _{u2p_y}$ is nearly constant, but the energy of $\sigma _{g2p_z}$ is raised considerably.
...
Note how the local coordinate system for nucleus B is left-handed, instead of the usual right-handed system like that of nucleus A. This is done so that the addition of AOs in constructive interference, while the subtraction of one AO from another results in destructive interference. 
...
With $\sigma_{g2s}$ as the baseline, the electron density of $\sigma_{g2s \, \vert \, \text{improved}}$ between the two nuclei is significantly larger. This in turn stablizes the MO.

On the other hand, compared to $\sigma_{g2p_{z}}$, the electron density of  $\sigma_{g2p_{z} \vert \, \text{improved}}$ between the two nuclei is significantly smaller. This in turn destablizes the MO.
...
[^1]: For all LCAO below, we've sneakily ignored AOs of $n = 3$ and above (ex. $3s$, $3p$, $3d$, $4s$, etc.). This is because AOs of higher energy contribute very little to the MOs we are interested in; these AOs are also unnecessary for our explanation.
